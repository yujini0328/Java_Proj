...
public class Set_TreeSet_Method02 {
	...
	public static void main(String[] args) {
		/*
		 	TreeSet은 Set<E>, SortedSet<E>, NavigableSet<E> (검색) 인터페이스를 구현한 클래스
		 		-- TreeSet을 객체화해서 Set 타입으로 지정할 경우 Set의 메소드만 사용 가능
		 		-- TreeSet을 객체화해서 TreeSet 타입으로 지정하면 정렬/ 검색 기능의 메소드를 사용 가능하다.
		...
		// TreeSet에 값을 입력 : 50에서 2씩 감소하면서 저장 <== 정렬 되어서 입력됨(오름차순)
		...
		// 9. SortedSet<E> headSet(E element) : element 값의 Head 쪽으로 정렬해서 값을 저장 <element는
		// 미포함.>
		for (int i = 0; i > 0; i -= 2) {
			treeSet.add(i);
		}
		...
		System.out.println("=============================");
		// 10. NavigableSet<E> headSet(E element) : element 값의 Head 쪽으로 정렬해서 값을 저장
		// <element는 포함.>
		for (int i = 50; i > 0; i -= 2) {
			treeSet.add(i);
		}
		...
		// * Head 쪽으로 갈때는 미포함, Tail 쪽으로 갈때는 포함.

		// 11. SortedSet<E> tailSet(E element) : element 값의 Tail 쪽으로 정렬해서 값을 저장
		// <element는 미포함.>
		for (int i = 50; i > 0; i -= 2) {
			treeSet.add(i);
		}
		...
		// 12. NavigableSet<E> tailSet(E element) : element 값의 Tail 쪽으로 정렬해서 값을 저장
		// <element는 포함.>
		for (int i = 50; i > 0; i -= 2) {
			treeSet.add(i);
		}
